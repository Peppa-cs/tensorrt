...
int picwidth = 32;
int picheight = 32;
string NetworkName = "vgg16cifar10baseline";
string InputTensorNames = "0";
...
ICudaEngine* networkToEngine(const BuildOptions& build, const SystemOptions& sys, IBuilder& builder, INetworkDefinition& network, std::ostream& err)
{
  ...
  /*  if ((build.int8 && build.calibration.empty()) || int8IO) 
    ...
    if (build.int8 || int8IO)
    {
        nvinfer1::Dims InputDims{3,3,picwidth,picheight};

    	BINcifarBatchStream calibrationStream(50, 10,InputDims);

        config->setInt8Calibrator(new Int8EntropyCalibrator2<BINcifarBatchStream>( calibrationStream, -1,  NetworkName.c_str(),
        InputTensorNames.c_str()));
    }
