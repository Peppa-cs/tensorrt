...
public:
        ...
        , mReadCache(readCache)
    {
        ...
        gLogError << "mCalibrationTableName: "  <<mCalibrationTableName<<std::endl;
    ...
    bool getBatch(void* bindings[], const char* names[], int nbBindings)
    {
        ...
        gLogError << "name[0]: "  <<names[0]<<std::endl;
        gLogError << "mInputBlobName: "  <<mInputBlobName<<std::endl;
    ...
    const void* readCalibrationCache(size_t& length)
    {
        ...
        std::ifstream input("/home/peppa3/"+mCalibrationTableName);
        input >> std::noskipws;
    ...
    {
        std::ofstream output("/home/peppa3/"+mCalibrationTableName);
        gLogError << "mCalibrationTable!!!!!!!"  <<std::endl;
        output.write(reinterpret_cast<const char*>(cache), length);
    }

    void returnvector(std::vector<char> &Cache)
    {
   
        Cache.assign(mCalibrationCache.begin(),mCalibrationCache.end());
    }
