...
struct SampleParams
{
    ...
    bool fp32{false};                  //!< Allow running the network in FP16 mode.
    bool verbose{true};
...
inline bool parseArgs(Args& args, int argc, char* argv[])
{
    while (1)
    {
	    ...
	    {"onnxFileName", required_argument, 0, 'o'},
            {"int8", no_argument, 0, 'i'},
            {"fp16", no_argument, 0, 'f'},
            {"fp32", no_argument, 0, 'l'},
            {"batch",required_argument,0,'b'},
            {"maxbatches",required_argument,0,'m'},
        ...
        case 'o':
            if (optarg)
            {
                args.onnxFileName.assign(optarg);
            }
            else
            {
                std::cerr << "ERROR: --onnxFileName requires option argument" << std::endl;
                return false;
            }
            break;
        case 'd':
        ...
        case 'm':
            if (optarg)
            {
               args.maxbatches = std::stoi(optarg);
            }
            break;
	case 'n':
            if (optarg)
